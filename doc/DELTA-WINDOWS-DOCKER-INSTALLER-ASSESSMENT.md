...
>
> **A§9.3's conditional sentence about uninstall is now a description of something that exists.** It reads *"Uninstall — if implemented at all — must require explicit typed confirmation and must offer a final backup first"*, and it was written when nothing of the kind was planned. Phase 12 implemented `uninstall.ps1` and satisfies both halves of that constraint: complete removal requires the word `DELETE` typed in full, and a verified final backup is offered first and written outside the installation root. The default mode destroys nothing. The text below is left as written — it is the record of what was assessed before any code existed — and the phasing document carries the implementation.
