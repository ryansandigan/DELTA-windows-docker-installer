...
function Invoke-DeltaCertificateManagement {
    <#
      ...
      Replaces the TLS certificate and key that NGINX serves, without
      recreating any container. The generated NGINX configuration names fixed
      files (certs\delta.crt and certs\delta.key, mounted read-only), so
      replacing the certificate is replacing those two files and telling NGINX
      to reload - the configuration itself does not change and neither does
      any container.

      The order is what makes it safe:

        validate  Phase 4's Test-DeltaCertificateMaterial, unchanged - parse,
                  not passphrase-protected, key matches certificate, dates.
                  A defect here is named and nothing is touched.
        back up   the certificate and key currently in place
        install   Phase 4's Install-DeltaCertificate, ACL and all
        nginx -t   against the live project, reading the new files
        reload    ONLY if nginx -t passed
        verify    NGINX still running, the endpoint answers, and the
                  certificate actually being served is the new one

      If nginx -t fails, the previous files are put back and NGINX is never
      signalled - it is still serving the old certificate from memory, so the
      site does not go down at any point.
    #>
    param(
        [Parameter(Mandatory)][string]$InstallRoot,
        [Parameter(Mandatory)][object]$Configuration,
    ...
    if (-not $Configuration.TlsEnabled) {
        Write-Host 'HTTPS is not enabled for this installation.'
        Write-Detail "TLS_ENABLED is false, so NGINX serves HTTP on port $($Configuration.HttpPort) and there is"
        Write-Detail 'no certificate in use to replace.'
        Write-Detail ''
        Write-Detail 'Enabling HTTPS changes the published ports and regenerates the NGINX configuration,'
        Write-Detail 'which is the installer flow rather than a configuration change:'
        Write-Detail ''
        Write-Detail '    .\setup.ps1 -Reconfigure'
        Write-Detail ''
        Write-Detail 'Nothing was changed.'
    ...
    $current = Get-DeltaInstalledCertificate -InstallRoot $InstallRoot
    Write-Host 'Certificate in use'
    if ($current.Exists -and $current.Thumbprint) {
        Write-Detail "Subject        $($current.Subject)"
        Write-Detail "Issuer         $($current.Issuer)"
        Write-Detail "Expires        $($current.NotAfter.ToString('yyyy-MM-dd')) ($($current.DaysRemaining) day(s) remaining)"
        Write-Detail "Thumbprint     $($current.Thumbprint)"
        Write-Detail "Files          $($current.CertificatePath)"
        Write-Detail "               $($current.KeyPath)"
    }
    elseif ($current.Exists) {
        Write-DeltaWarning $current.Reason
    }
    else {
    ...
    while ($true) {
        if (-not $CertificatePath -or -not $KeyPath) {
            if (-not $AllowPrompt) {
                $result.Reason = 'A replacement certificate and key are required, and this run is non-interactive. Nothing was changed.'
                return $result
            }
            Write-Host ''
            Write-Host 'Supply the replacement certificate and private key (PEM format).'
            Write-Host 'Leave either blank to cancel.'
            Write-Host ''
            $CertificatePath = ([string](Read-Host -Prompt 'Certificate file (.crt/.pem)')).Trim('"', ' ')
            if (-not $CertificatePath) {
                $result.Cancelled = $true; $result.Stage = 'cancelled'
                $result.Reason = 'Cancelled. The certificate in use is unchanged.'
                return $result
            }
            $KeyPath = ([string](Read-Host -Prompt 'Private key file (.key/.pem)')).Trim('"', ' ')
            if (-not $KeyPath) {
                $result.Cancelled = $true; $result.Stage = 'cancelled'
                $result.Reason = 'Cancelled. The certificate in use is unchanged.'
                return $result
        ...
        Write-Host ''
        Write-Step 'Validating the replacement certificate'
        $validation = Test-DeltaCertificateMaterial -CertificatePath $CertificatePath -KeyPath $KeyPath -OpenSslImage $openSslImage
        $result.Validation = $validation

        if ($validation.IsValid) {
            Write-Detail "[ ok ]     subject $($validation.Subject)"
            Write-Detail "[ ok ]     the private key matches the certificate"
            Write-Detail "[ ok ]     valid until $($validation.NotAfter.ToString('yyyy-MM-dd'))"
            if ($validation.Warning) { Write-DeltaWarning $validation.Warning }
            break
        }

        Write-DeltaFailure ''
        Write-DeltaFailure 'The replacement certificate was rejected.'
        Write-Detail $validation.Reason
        Write-Detail ''
        Write-Detail 'Nothing has been changed - the certificate currently in use is untouched.'
        if (-not $AllowPrompt) {
        ...
        if (-not (Read-DeltaYesNoConfirmation -Body { Write-Host 'Try a different certificate?' })) {
            $result.Cancelled = $true; $result.Stage = 'cancelled'
            $result.Reason = "Cancelled after a validation failure: $($validation.Reason)"
            return $result
        }
        $CertificatePath = $null; $KeyPath = $null
    }

    $result.ExpectedThumbprint = $result.Validation.Thumbprint
    ...
    $test = Test-DeltaNginxConfiguration -InstallRoot $InstallRoot -ProjectName $Configuration.ProjectName
    $result.NginxTest = $test

    if ($test.Tested -and -not $test.Succeeded) {
        Write-DeltaFailure ''
        Write-DeltaFailure 'NGINX rejected the configuration with the new certificate. It has NOT been reloaded.'
        foreach ($line in ($test.Output -split "`r?`n")) { if ($line.Trim()) { Write-Detail "  $line" } }
        Write-Detail ''
        Write-Step 'Restoring the previous certificate'
        $result.Restored = Restore-DeltaCertificateMaterial -InstallRoot $InstallRoot -Backup $result.Backup
        if ($result.Restored) {
            $recheck = Test-DeltaNginxConfiguration -InstallRoot $InstallRoot -ProjectName $Configuration.ProjectName
            if ($recheck.Succeeded) {
                Write-Success 'The previous certificate is back in place and NGINX validates it.'
                Write-Detail 'NGINX was never reloaded, so it has been serving the previous certificate throughout.'
            }
            else {
                Write-DeltaWarning 'The previous certificate was put back but NGINX does not validate it either.'
                Write-Detail $recheck.Output
            }
        }
        else {
            ...
            Write-Detail 'NGINX has not been reloaded, so it is still serving the previous certificate from memory.'
        }
        $result.Reason = 'nginx -t rejected the new certificate.'
        return $result
    }
    if (-not $test.Tested) {
        Write-DeltaWarning $test.Reason
    }
    else {
        Write-Detail '[ ok ]     nginx -t passed'
    }
    ...
    if (-not $result.Reloaded) {
        ...
        if ($result.Restored) {
            ...
            Write-Detail 'The previous certificate has been put back and a reload attempted with it.'
        }
    ...
    Write-Detail '[ ok ]     reload accepted; the NGINX container was not recreated'

    # --- verify -----------------------------------------------------------
    $result.Stage = 'verify'
    Start-Sleep -Seconds 2
    $url = (Get-DeltaPublicUrl -Scheme 'https' -HostName 'localhost' -Port ([int]$Configuration.HttpsPort)) + '/'
    $result.Http = Test-DeltaHttpEndpoint -Url $url -TimeoutSeconds 30
    $result.ServedThumbprint = Get-DeltaServedCertificateThumbprint -HostName 'localhost' -Port ([int]$Configuration.HttpsPort)

    if (-not $result.Http.Succeeded) {
        Write-DeltaFailure ''
        Write-DeltaFailure "The new certificate was installed and NGINX reloaded, but $url did not answer."
        Write-Detail $(if ($result.Http.Error) { $result.Http.Error } else { "HTTP $($result.Http.StatusCode)" })
        Write-Detail ''
        Write-Step 'Restoring the previous certificate'
        $result.Restored = Restore-DeltaCertificateMaterial -InstallRoot $InstallRoot -Backup $result.Backup
        if ($result.Restored -and (Invoke-DeltaNginxReload -InstallRoot $InstallRoot -ProjectName $Configuration.ProjectName)) {
            $back = Test-DeltaHttpEndpoint -Url $url -TimeoutSeconds 30
    ...
    Write-Detail "[ ok ]     GET $url returned $($result.Http.StatusCode)"
    if ($result.ServedThumbprint) {
        if ($result.ServedThumbprint -eq $result.ExpectedThumbprint) {
            Write-Detail "[ ok ]     the certificate being served is the new one ($($result.ServedThumbprint))"
        }
        else {
            Write-DeltaWarning "NGINX is serving thumbprint $($result.ServedThumbprint), but the new certificate is $($result.ExpectedThumbprint)."
    ...
    # --- record -----------------------------------------------------------
    # The existing Phase 4 fields, updated in place. No thumbprint is a
    # secret; no path outside the installation and no key material is stored.
    try {
        Write-DeltaInstallState -InstallRoot $InstallRoot -Properties ([ordered]@{
            certificateThumbprint = $result.Validation.Thumbprint
            certificateNotAfter   = $result.Validation.NotAfter.ToUniversalTime().ToString('yyyy-MM-ddTHH:mm:ssZ')
            tlsMode               = 'supplied'
        })
    }
    catch {
...
function Invoke-DeltaCertificateOperation {
    <#
      Menu option 7's reporting wrapper.
    #>
    param(
        [Parameter(Mandatory)][string]$InstallRoot,
        [Parameter(Mandatory)][object]$Configuration,
    ...
    $outcome = Invoke-DeltaCertificateManagement -InstallRoot $InstallRoot -Configuration $Configuration -AllowPrompt $AllowPrompt
    if ($outcome.Stage -eq 'tls-disabled') { return $outcome }

    Write-Host ''
    if ($outcome.Succeeded) {
        Write-Success 'Certificate replaced.'
        Write-Detail "Subject        $($outcome.Validation.Subject)"
        Write-Detail "Expires        $($outcome.Validation.NotAfter.ToString('yyyy-MM-dd'))"
        Write-Detail "Thumbprint     $($outcome.Validation.Thumbprint)"
        if ($outcome.Backup.CertificateBackup) { Write-Detail "Previous saved $($outcome.Backup.CertificateBackup)" }
        Write-Detail ''
        Write-Detail 'No container was recreated - NGINX reloaded in place, and DELTA and the database'
        Write-Detail 'were not involved at all.'
    }
    elseif ($outcome.Cancelled) {
        Write-Detail 'Cancelled. The certificate in use is unchanged.'
    }
    else {
        Write-DeltaFailure 'The certificate was not replaced.'
        Write-Detail "Stage reached  $($outcome.Stage)"
        Write-Detail $outcome.Reason
